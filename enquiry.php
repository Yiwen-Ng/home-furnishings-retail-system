...
// Fetch user's enquiries with product names
try {
    $stmt = $pdo->prepare("
        SELECT e.*, p.prod_Name 
        FROM enquiries e 
        JOIN products p ON e.product_id = p.prod_Id 
        WHERE e.user_id = ? 
        ORDER BY e.created_at $order_by
    ");
    $stmt->execute([$user_id]);
    $enquiries = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    error_log("Error fetching enquiries: " . $e->getMessage());
    $enquiries = [];
    $error = "Failed to load enquiries. Please try again later.";
}

// Fetch user data (for consistency with profile.php)
    ...
    <div class="main-content">
        ...
        <div class="container">
            ...
            <?php elseif (empty($enquiries)): ?>
                <p class="no-enquiries">You have no enquiries yet.</p>
            <?php else: ?>
                <p class="summary-text">You have <?php echo count($enquiries); ?> enquiries.</p>

                <div class="filter-tabs">
                    <a href="?sort=desc" class="tab <?php echo $sort === 'desc' ? 'active' : ''; ?>">Latest</a>
                    <a href="?sort=asc" class="tab <?php echo $sort === 'asc' ? 'active' : ''; ?>">Oldest</a>
                </div>
                ...
                <table class="order-table">
                    ...
                    <tbody>
                        <?php foreach ($enquiries as $enquiry): ?>
                            <?php
                                $material = $enquiry['custom_material'] ?? '';
                                if (empty($material) || $material === '[]') {
                                    $material_display = 'N/A';
                                } else {
                                    $material_display = htmlspecialchars($material);
                                }

                                $color = $enquiry['custom_color'] ?? '';
                                $color_display = empty($color) ? 'N/A' : htmlspecialchars($color);
                                $color_square = empty($color) ? '' : '<span style="display:inline-block; width:20px; height:20px; background-color:' . htmlspecialchars($color) . '; border:1px solid #ccc; margin-left:10px;"></span>';
                            ?>
                            <tr>
                                <td><?php echo date('Y-m-d H:i', strtotime($enquiry['created_at'])); ?></td>
                                <td><a href="product.php?product_id=<?php echo $enquiry['product_id']; ?>" class="product-link"><?php echo htmlspecialchars($enquiry['prod_Name']); ?></a></td>
                                <td><?php echo htmlspecialchars($enquiry['custom_dimensions'] ?? 'N/A'); ?></td>
                                <td><?php echo $material_display; ?></td>
                                <td><?php echo $color_display . $color_square; ?></td>
                                <td><?php echo htmlspecialchars(ucfirst($enquiry['contact_preference'])); ?></td>
                                <td><?php echo nl2br(htmlspecialchars($enquiry['additional_notes'] ?? 'N/A')); ?></td>
                                <td>
                                    <?php if ($enquiry['file_path']): ?>
                                        <a href="<?php echo htmlspecialchars($enquiry['file_path']); ?>" class="btn-view" target="_blank">View File</a>
                                    <?php else: ?>
                                        N/A
                                    <?php endif; ?>
                                </td>
